...
REFLECTIVE_LOG = LOGS_DIR / "reflective" / "archy_activity.log"
...
def federation_listener(agent_name: str = "Archy") -> None:
    ...
    # Reflective runtime log (normalization)
    _append_line(REFLECTIVE_LOG, f"[{_iso_now()}] Archy online — federation listener active")
        ...
        for task_file in tasks_dir.glob("*.json"):
            try:
                ...
                if str(data.get("target")) == agent_name:
                    print(f"[" + agent_name + f"] Received task: {task_file.name}")
                    response = {
                        "agent": agent_name,
                        "received": _iso_now(),
                        "status": "ok",
                ...
                print(f"[" + agent_name + f"] Error handling {task_file.name}: {e}")
        time.sleep(2)
