...
def curate(source_path: Optional[str] = None, curator: Optional[str] = None) -> None:
    """Librarius — Knowledge Curator (v1.0)
    ...
    - Integrates ModelRouter for governance-visible summarization (primary: gpt-5-mini, fallback: oss-safeguard)
    """
    load_dotenv()
    if source_path:
    ...
    # Governance-visible summarization via ModelRouter (logs compliance + event)
    try:
        from agents.model_router import ModelRouter  # lazy import
        router = ModelRouter(agent_name="Librarius")
        meta = {"risk": "low", "tokens": 2000, "task_id": "lib-ingest", "phase": "38.7"}
        summary_prompt = (
            f"Summarize ingestion outcome: files={len(files)}, chunks={total_chunks}. "
            "Provide a concise 1-2 sentence description."
        )
        _ = router.route(meta, summary_prompt)
    except Exception:
        # best-effort logging only; ignore failures to keep ingestion deterministic
        pass
